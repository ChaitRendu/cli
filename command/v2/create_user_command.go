...
type CreateUserActor interface {
	CreateUser(username string, password string) (v2action.Warnings, error)
}
...
func (cmd *CreateUserCommand) Execute(args []string) error {
	...
	warnings, err := cmd.Actor.CreateUser(cmd.RequiredArgs.Username, cmd.RequiredArgs.Password)
